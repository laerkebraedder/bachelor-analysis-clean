...
pp_check(s_c_m1.1_prior, nsamples = 100) +
  pp_check(s_c_m1.1, nsamples = 100)
...
pp_check(s_n_m1.1_prior, nsamples = 100) +
  pp_check(s_n_m1.1, nsamples = 100)
...
pp_check(s_b_m1.1_prior, nsamples = 100)
...
pp_check(s_b_m1.1_prior, nsamples = 100) +
  pp_check(s_b_m1.1, nsamples = 100)
...
# Create summary dataset for visualisation
exp1_plotSum_c2 <- dfs %>% group_by(ID, Complexity_num, Orientation) %>% summarise(
  rt_sum = mean(RT)
)
exp1_plotSum_n2 <- dfs %>% group_by(ID, Noise_num, Orientation) %>% summarise(
  rt_sum = mean(RT)
)
exp1_plotSum_b2 <- dfs %>% group_by(ID, BlankSpace_num, Orientation) %>% summarise(
  rt_sum = mean(RT)
)


exp1_plotSum_c2$Orientation <- as.factor(exp1_plotSum_c2$Orientation)
exp1_plotSum_n2$Orientation <- as.factor(exp1_plotSum_n2$Orientation)
exp1_plotSum_b2$Orientation <- as.factor(exp1_plotSum_b2$Orientation)


ggplot(exp1_plotSum_c2, aes(Complexity_num, rt_sum, color = Orientation)) + 
  geom_point()+
...
ggplot(exp1_plotSum_n2, aes(Noise_num, rt_sum, color = Orientation)) + 
  geom_point()+
  geom_smooth(method=lm) +
  scale_x_continuous(breaks = c(1, 2, 3),labels=c("Low", "Medium", "High")) +
  labs(title="\nExperiment 1",
...
ggplot(exp1_plotSum_b2, aes(BlankSpace_num, rt_sum, color = Orientation)) + 
  geom_point()+
  geom_smooth(method=lm) +
  scale_x_continuous(breaks = c(1, 2, 3),labels=c("Low", "Medium", "High")) +
  labs(title="\nExperiment 1",
       subtitle="Saliency",
       x=expression("Blank space"),
       y=expression("Reaction time"))
