...
pp_check(m_c_m2.1_prior, nsamples=100)
...
#Predictive checks
pp_check(m_c_m2.1_prior, nsamples=100) + 
  pp_check(m_c_m2.1, nsamples=100)
...
# prior predictive check
pp_check(m_n_m2.1_prior, nsamples=100)
...
#Predictive checks
pp_check(m_n_m2.1_prior, nsamples=100) + 
  pp_check(m_n_m2.1, nsamples=100)
...
# prior predictive check
pp_check(m_b_m2.1_prior, nsamples=100)
...
#Predictive checks
pp_check(m_b_m2.1_prior, nsamples=100) + 
  pp_check(m_b_m2.1, nsamples=100)
...
```{r Orientation}
df <- read_csv("data/ImageSimilarityData.csv")

ggplot(df, aes(complex, MSE, color = orientation)) + 
  geom_point() +
...
ggplot(df, aes(noise, MSE, color = orientation)) + 
  geom_point() +
  geom_smooth(method = lm) + 
  labs(x = "Noise")

ggplot(df, aes(blank, MSE, color = orientation)) + 
  geom_point() +
  geom_smooth(method = lm) + 
  labs(x = "Blank Space")
